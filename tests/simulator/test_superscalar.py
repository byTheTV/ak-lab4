...
def test_superscalar_nop_nop_advances_pc_by_two() -> None:
    ...
    dm = [0] * 16
    machine = Machine(im=im, dm=dm, pc=0, sp=STACK_BASE, superscalar=True)
...
def test_log_par_line_on_dual_issue() -> None:
    ...
    buf = StringIO()
    machine = Machine(im=im, dm=dm, pc=0, sp=STACK_BASE, superscalar=True)
    run_program(machine, max_ticks=100, log=buf)
...
def test_dual_blocked_by_control_on_second() -> None:
    ...
    buf = StringIO()
    machine = Machine(im=im, dm=dm, pc=0, sp=STACK_BASE, superscalar=True)
    run_program(machine, max_ticks=200, log=buf)
    lines = buf.getvalue().strip().splitlines()
...
def test_deferred_store_visible_only_after_flush_on_halt() -> None:
    ...
    machine.step()  # push addr + push value
    while machine.pc < 3:
        machine.step()  # stall/issue до момента после STORE
    assert machine.dm[addr] == 0
    assert machine.shadow_stores == [(addr, value)]

    run_program(machine, max_ticks=200)  # halt -> flush
    assert machine.halted
    assert machine.dm[addr] == value
    assert machine.shadow_stores == []
...
def test_irq_delivery_flushes_shadow_before_isr() -> None:
    ...
    machine.step(log=buf)  # jmp 10
    machine.step(log=buf)  # push pair
    while machine.pc < 13:
        machine.step(log=buf)  # stall/issue до STORE
    assert machine.shadow_stores == [(15, 55)]
    machine.irq_pending[0] = True
    machine.irq_line_value[0] = 65
    machine.step(log=buf)  # irq delivery + flush shadow

    assert machine.pc == 20
    assert machine.interrupt_depth == 1
    assert machine.dm[15] == 55
    assert machine.shadow_stores == []
